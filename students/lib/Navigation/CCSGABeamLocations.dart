...
const List<String> _conversationPath = ['conversations/:conversationId'];
...
class ConversationLocation extends BeamLocation {
  ...
  @override
  List<BeamPage> get pages => [
        if (pathSegments.contains('conversations'))
          BeamPage(
        ...
        if (pathParameters.containsKey('conversationId'))
          BeamPage(
            key: ValueKey('conversation-${pathParameters['conversationId']}'),
            child: ConversationPage(
                conversationId: int.parse(pathParameters['conversationId'])),
          )
      ];
}
